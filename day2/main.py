...
dummy_values = ["1-3 a: abcde", "1-3 b: cdefg", "2-9 c: ccccccccc"]


def find_valid_password_counts_part_1():
    valid = []
    for value in test_values:
        matches = re.search(regex, value)
        minimum = int(matches.group("minimum"))
        maximum = int(matches.group("maximum"))
        required = matches.group("required").replace(":", "")
        password = matches.group("password")

        if minimum <= password.count(required) <= maximum:
            valid.append(password)
    return len(valid)


def find_valid_password_counts_part_2():
    valid = []
    for value in test_values:
        matches = re.search(regex, value)
        minimum = int(matches.group("minimum"))
...
if __name__ == "__main__":
    print(find_valid_password_counts_part_1())
    print(find_valid_password_counts_part_2())
